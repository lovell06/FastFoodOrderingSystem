...
public class VerifyForgotPasswordHandler : IHandler<VerifyForgotPasswordCommand, Result<Unit>>
{
    private readonly IForgotPasswordOtpStore _forgotPasswordOtpStore;
    private readonly ILogger<VerifyForgotPasswordHandler> _logger;
    ...
    public VerifyForgotPasswordHandler(
        IForgotPasswordOtpStore forgotPasswordOtpStore, 
        ILogger<VerifyForgotPasswordHandler> logger, 
    ...
    public async Task<Result<Unit>> HandleAsync(VerifyForgotPasswordCommand command, CancellationToken cancellationToken)
    {
        ...
        if (forgotPasswordOtp is null)
        {
            ...
            return Result<Unit>.Failure(ForgotPasswordError.Unauthorized);
        }
